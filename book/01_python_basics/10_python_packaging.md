...
Chúng ta có thể tạo dự án (package) mới với nhiều công cụ khác nhau, nhưng trong bài học này chúng ta sẽ sử dụng thư viện `poetry` cho quản lý và đóng gói Python. Cụ thể chúng ta sẽ tạo ra 1 gói `geomath` - một thư viện đơn giản cho việc tính toán khoảng cách giữa hai điểm sử dụng công thức haversine. Nếu bạn chưa cài đặt `poetry`, bạn có thể cài đặt như sau `pip install poetry`.
...
Để tạo một dự án/gói python mới, chúng ta sử dụng câu lệnh `poetry new tengoi`.
...
## 10.3. Quản lý phụ thuộc với Poetry
...
Thư viện tiện ích tính toán khoảng cách địa lý.

## Cài đặt

```bash
pip install git+https://github.com/yourusername/geomath.git
```
## Sử dụng nhanh

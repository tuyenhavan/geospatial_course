...
> **Lưu ý**: Bạn có thể chạy trực tiếp notebook này bằng **Google Colab** thông qua [liên kết này](https://colab.research.google.com/drive/1LOh-mvecOfpXbrWwOM7dRP1epV9zUl34) mà không cần cài đặt Python.
...
### 3.2.1. Tạo danh sách
'Để tạo danh sách trong Python, bạn sử dụng dấu ngoặc vuông `[]` và phân tách các phần tử bằng dấu phẩy. Các ví dụ sau mô tả tạo danh bằng các phương thức khác nhau.

- **Tạo danh sách sử dụng dấu ngoặc vuông**

Tạo danh sách thành phố sử dụng dấu ngoặc vuông. 
...
Danh sách trong Python có thể chứa các loại dữ liệu khác nhau, bao gồm chuỗi, số nguyên, số thực, giá trị boolean và thậm chí là None như ví dụ bên dưới. 
...
Một cách tạo ra danh sách thuận tiện là dùng từ khóa `list` kêt hợp hàm `range` để tại ra một danh sách số theo mong muốn.
...
Một phương thức phổ biến để thêm phần tử mới vào danh sách là sử dụng phương thức `append()` như ví dụ bên dưới.
...
Có nhiều cách để xóa phần tử khỏi danh sách trong Python. Bạn có thể sử dụng `del` để xóa phần tử tại một chỉ mục cụ thể, `remove()` để xóa phần tử có giá trị cụ thể, `pop()` để xóa phần tử cuối cùng hoặc một chỉ mục cụ thể, `clear()` để xóa tất cả phần tử nhưng giữ lại danh sách, hoặc `del` để xóa hoàn toàn danh sách khỏi bộ nhớ. 
...
Để đếm số phần tử trong một danh sách, bạn có thể sử dụng hàm `len()`.
...
## 3.3. Bộ giá trị
Bộ giá trị (tuple) là tập hợp có thứ tự và không thể thay đổi. Sử dụng chúng cho dữ liệu cố định, như tọa độ, và được tạo ra dùng dấu ngoặc đơn `()` hoặc từ khóa `tuple()`.

### 3.3.1. Tạo tuple

- **Tạo `tuple` dùng dấu ngoặc đơn hoặc từ khóa `tuple`**

Tuple có thể tạo ra bằng cách sử dụng dấu ngoặc đơn hoặc từ khóa tuple. bạn có thể tạo một tuple chứa tọa độ của một địa điểm như ví dụ bên dưới.
...
Để tạo một tuple một phần tử, bạn cần thêm dấu phẩy sau phần tử đó. Nếu không có dấu phẩy, Python sẽ hiểu đó là một giá trị đơn giản chứ không phải là một tuple.
...
### 3.3.2. Đếm phần tử trong tuple

Các đơn giản nhất để đếm số phần tử trong một tuple là sử dụng hàm `len()`.


```python
# Thêm thành phố mới
points = (20, 30, 40)
print(f"Số lượng phần tử trong tuple: {len(points)}") # Sử dụng hàm len() để đếm số phần tử trong tuple
```

    Số lượng phần tử trong tuple: 3
    

## 3.4. Từ điển 
Từ điển (dictionary) lưu trữ các cặp khóa-giá trị. Sử dụng chúng cho dữ liệu có cấu trúc, và được tạo ra sử dụng dấu ngoặc nhọn `{}` hoặc từ khóa `dict()`.

### 3.4.1. Tạo dictionary 

- **Tạo từ điển từ dấu ngoặc nhọn `{}`**

Từ điển lưu trữ dữ liệu theo kiểu cặp giá trị (key:value). Cú pháp cơ bản như sau:
...
Từ điển có thể được tạo ra bằng cách sử dụng từ khóa dict. Ví dụ bên dưới sử dụng từ khóa `dict` để tạo ra từ điển với cặp khóa và giá trị tương ứng.
...
Từ điển lồng nhau là từ điển bên trong từ điển như ví dụ bên dưới.
...
Cách phổ biến để truy cập giá trị trong từ điển là sử dụng khóa tương ứng như trong ví dụ bên dưới.
...
Để cập nhật giá trị trong từ điển, chúng ta dùng khóa cũ và gán giá trị mới cho khóa đó như bên dưới.
...
Ta muốn thêm cặp khóa:giá trị bằng cách tạo ra khóa mới và thêm giá trị vào như ví dụ bên dưới.
...
Để xem tất cả các khóa trong từ điển, ta dùng hàm `.keys()` để tạo ra một danh sách các khóa trong từ điển đó.
...
Ta có thể xóa một khóa và giá trị cụ thể hoặc toàn bộ từ điển như ví dụ bên dưới.
...
## 3.5. Tập hợp 
Tập hợp (set) là tập hợp không có thứ tự các phần tử duy nhất. Sử dụng chúng để loại bỏ trùng lặp hoặc kiểm tra thành viên. Chúng được tạo ra sử dụng dấu ngoặc nhọn `{}` hoặc từ khóa `set()`.

### 3.5.1. Tạo tập hợp

- **Tạo `set` bằng dấu ngoặc nhọn**

Cách đơn giản để tạo 1 `set` là dùng dấu ngoặc nhọn như ví dụ bên dưới.
...
Ngoài ra, ta có thể tạo `set` bằng từ khóa `set()` và cho vào 1 danh sách hoặc tuple.
...
Giao của hai tập hợp là tập hợp chứa các phần tử xuất hiện trong cả hai tập hợp. Bạn có thể tính giao của hai tập hợp bằng cách sử dụng toán tử `&` hoặc phương thức `intersection()`.
...
bạn có thể xóa một phần tử cụ thể trong tập hợp hoặc toàn bộ tập hợp như ví dụ bên dưới.

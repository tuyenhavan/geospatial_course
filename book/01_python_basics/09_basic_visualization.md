...
import numpy as np
print('Phiên bản Matplotlib:', plt.matplotlib.__version__)
```
...
populations = [8053663, 9420000, 1134000]
...
ax.set_title('Vị trí các thành phố Việt Nam')
...
for i, ax in enumerate(axes.flatten()):
    if i ==0:
        ...
        y = df['species'].unique()
        ax.boxplot(x, tick_labels=y)
        ax.set_xlabel('Loài hoa', fontsize=12)
        ax.set_ylabel('Chiều dài đài hoa (cm)', fontsize=12)
        ...
        ax.set_xlabel('Chiều rộng đài hoa (cm)', fontsize=12)
    ...
    if i==0:
        for j, specie in enumerate(species):
            ...
            ax.scatter(subset['sepal_length'], subset['sepal_width'], 
                       color=colors[j], label=specie, alpha=0.7)
        ax.set_xlabel('Chiều dài đài hoa (cm)', fontsize=12)
        ax.set_ylabel('Chiều rộng đài hoa (cm)', fontsize=12)
        ax.legend(title='Loài hoa')
        ax.tick_params(axis='both', which='major', labelsize=12)
        ax.text(0.07, 0.9, '(a)', transform=ax.transAxes, fontsize=12)
    else:
        # Biểu đồ histogram 
        ...
        ax.set_ylabel('Tần suất', fontsize=12)
        ax.tick_params(axis='both', which='major', labelsize=12)
        ax.text(0.07, 0.9, '(b)', transform=ax.transAxes, fontsize=12)
```
...
ax.set_xticks(np.linspace(0, len(heat.columns) - 1, 10))
ax.set_xticklabels(heat.columns[np.linspace(0, len(heat.columns) - 1, 10).astype(int)])
ax.set_yticks(np.arange(len(heat.index)))
ax.set_yticklabels(heat.index)
ax.tick_params(axis='both', which='major', labelsize=12)
ax.set_ylabel('Bang', fontsize=12)
...
cbar.set_label('Số ca bệnh sởi', fontsize=12)
```

...
```python
import matplotlib.pyplot as plt # Nếu bạn chưa cài đặt matplotlib, hãy chạy: pip install matplotlib hoặc xem lại Bài 1.
...
Biểu đồ cột là một cách trực quan để so sánh các giá trị giữa các nhóm khác nhau. Trong ví dụ này, chúng ta đã tạo một biểu đồ cột để so sánh dân số của ba thành phố lớn ở Việt Nam. Mỗi cột đại diện cho một thành phố và chiều cao của cột tương ứng với dân số của thành phố đó.
...
Biểu đồ phân tán dùng để thể hiện mối quan hệ giữa hai biến số. Bằng cách sử dụng các điểm trên biểu đồ, bạn có thể dễ dàng nhận biết xu hướng, phân bố, và các điểm ngoại lệ trong dữ liệu của mình.
...
Kết hợp nhiều biểu đồ trong một figure cho phép bạn so sánh và phân tích dữ liệu từ nhiều góc độ khác nhau trong cùng một không gian trực quan.
...
# Giới hạn NDVI trong khoảng [0,1]
...
fig, ax = plt.subplots(figsize=(10, 5))
# Thêm năm vào biểu đồ hexbin bằng cách sử dụng màu sắc khác nhau cho mỗi năm
hb = ax.hexbin(temperature, ndvi, gridsize=30, cmap='inferno', mincnt=1, C=years, reduce_C_function=np.mean)
ax.set_xlabel('Temperature (°C)', fontsize=12)
ax.set_ylabel('NDVI', fontsize=12) # Thêm nhãn cho trục y
cbar = fig.colorbar(hb, ax=ax, extend="both", pad=0.03)
years = np.arange(2000, 2021)
cbar.set_ticks(np.arange(2000, 2021, 4))
cbar.set_ticklabels(np.arange(2000, 2021, 4))
cbar.set_label('Year', fontsize=12) # Thêm nhãn cho colorbar
cbar.ax.tick_params(labelsize=12) # Tùy chỉnh kích thước chữ trên colorbar
ax.grid(linestyle='--', alpha=0.5, color="gray") # Thêm lưới với kiểu đường nét đứt và độ mờ
plt.show()
```

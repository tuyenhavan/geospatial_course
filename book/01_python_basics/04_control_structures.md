...
> **Lưu ý**: Bạn có thể chạy trực tiếp notebook này bằng **Google Colab** thông qua [liên kết này](https://colab.research.google.com/drive/12lMlI6uiW5EYI2sbEjvy8E2Xt9UYZp1S). 
...
Vòng lặp cho phép chúng ta lặp lại hành động một cách hiệu quả, rất cần thiết để xử lý nhiều điểm dữ liệu. Trong phần này, chúng ta sẽ tìm hiểu vòng lặp `for` và `while`.
...
Vòng lặp `for` có thể được sử dụng để lặp qua một dãy số bằng cách sử dụng hàm `range()`. Hàm `range()` tạo ra một dãy số từ start đến end (không bao gồm end) với bước nhảy step.
...
Vòng lặp `for` có thể được sử dụng để lặp qua các phần tử trong một danh sách. 
...
Vòng lặp `for` sử dụng `enumerate()` cho phép bạn lặp qua một danh sách và đồng thời có được chỉ số index của phần tử hiện tại. Điều này rất hữu ích khi bạn cần biết vị trí của phần tử trong danh sách.
...
Khi lặp qua một dictionary, bạn có thể sử dụng phương thức `.items()` để lấy cả khóa và giá trị cùng lúc hoặc `.keys()` chỉ lấy khóa hoặc `.values()` chỉ lấy giá trị. Ví dụ sau dùng phương thức `.items()` để lặp qua cả khóa và giá trị.

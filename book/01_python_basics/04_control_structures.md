...
if wind_speed < 5:
    wind_category = "Lặng gió"
    warning = "Tốt cho hoạt động ngoài trời"
elif wind_speed < 20:
    wind_category = "Gió nhẹ"
    warning = "Điều kiện tốt"
elif wind_speed < 40:
    wind_category = "Gió vừa"
    warning = "Cẩn thận khi ra ngoài"
elif wind_speed < 60:
    wind_category = "Gió mạnh"
    warning = "Tránh hoạt động ngoài trời"
else:
    wind_category = "Gió rất mạnh"
    warning = "Ở trong nhà!"

print(f"Tốc độ gió: {wind_speed} km/h")
print(f"Phân loại: {wind_category}")
...
if latitude > 0:  # Bán cầu Bắc
    hemisphere = "Bắc"
    if season == "winter":
        daylight_hours = "ngắn"
        temperature_trend = "lạnh"
    else:  # mùa hè
        daylight_hours = "dài"
        temperature_trend = "ấm"
else:  # Bán cầu Nam
    hemisphere = "Nam"
    if season == "winter":
        daylight_hours = "dài"
...
print(f"Vị trí: {latitude}° vĩ độ (Bán cầu {hemisphere})")
print(f"Mùa: {season}")
print(f"Thời gian ánh sáng dự kiến: {daylight_hours}")
print(f"Xu hướng nhiệt độ: {temperature_trend}")
```
...
if elevation_a > elevation_b:
    ...
    higher_location = "Cả hai địa điểm đều có độ cao bằng nhau"
print(f"Địa điểm cao hơn: {higher_location}")
```
...
# Toán tử AND
comfortable_weather = temperature >= 20 and temperature <= 25 and humidity < 70
...
print("\nSử dụng zip với dữ liệu thực (Pythonic hơn):")
...
for index, city in enumerate(cities):
    ...
    # print(f"  Dân số: {populations[index]:,} người")
```
...
while sample_size < 1000:  # Ngăn vòng lặp vô hạn
...
Xử lý lỗi giúp làm cho chương trình của bạn mạnh mẽ hơn khi gặp phải tình huống không mong muốn.
...
```python
# Error handling for coordinate validation
...
```python
human_height = 1.75  # Chiều cao hợp lệ
try:
    assert human_height > 0, "Chiều cao phải là số dương"
    print(f"Chiều cao hợp lệ: {human_height}m")
except AssertionError as he:
    print(f"Lỗi xác thực chiều cao: {he}")
```

...
> **Lưu ý**: Bạn có thể chạy trực tiếp notebook này bằng **Google Colab** thông qua [liên kết này](https://colab.research.google.com/drive/1dwiHzvHLSXzoO6LzmQoqBxC_c5H6hEzv) mà không cần cài đặt Python. Trong trường hợp bạn muốn tạo bản sao của notebook này, bạn có thể làm như sau: `File → Save a copy in Drive`.
...
Hàm (function) là blocks code có thể tái sử dụng để thực hiện một tác vụ cụ thể. Hàm được xây dựng bắt đầu bằng từ khóa `def`, theo sau là tên hàm và dấu ngoặc đơn. Hàm có thể có tham số (một hoặc nhiều) hoặc không và có kết quả trả về hoặc không. Nếu hàm không có tham số, dấu ngoặc đơn sẽ để trống. Cấu trúc cơ bản của một hàm đơn giản như sau:

```bash
def ten_ham(thamso1, thamso2):
    # Nội dung của hàm
    # Có thể có nhiều dòng mã ở đây
    # Cuối cùng, có thể có một câu lệnh return để trả về giá trị (nếu cần)
    # Nếu không có câu lệnh return, hàm sẽ trả về None theo mặc định
    return ketqua # Nếu kết quả được gán vào biến ketqua
```

### 5.2.1. Hàm có và không có tham số
...
```python
# Hàm với tham số (parameters). Trong trường hợp này là một tham số tên (name)
...
Hàm có thể trả về một hoặc nhiều giá trị cùng lúc.
...
### 5.2.4. Hàm với tham số biến đổi

Hàm có thể nhận số lượng arguments linh hoạt sử dụng `*args` và `**kwargs`.

- **Hàm với positional arguments `(*args)`**

Hàm `*args` được sử dụng khi bạn muốn truyền một số lượng không giới hạn các đối số vị trí vào một hàm. Trong ví dụ dưới, chúng ta có thể truyền bất kỳ số lượng nào của các số vào hàm display_number, và nó sẽ hiển thị chúng dưới dạng một chuỗi. Bản chất của `*args` là một tuple, vì vậy bạn có thể xử lý nó như một tuple trong hàm. Cú pháp như sau:
...
Hàm `**kwargs` được sử dụng khi bạn muốn truyền một số lượng không giới hạn các đối số từ khóa (keyword arguments) vào một hàm. Trong ví dụ dưới, chúng ta có thể truyền bất kỳ số lượng nào thông tin về địa điểm vào hàm create_location_info, và nó sẽ tạo ra một dictionary chứa tất cả các thông tin đó. Bản chất của `**kwargs` là một dictionary, vì vậy bạn có thể xử lý nó như một dictionary trong hàm. Cú pháp như sau:
...
Hàm Lambda là một hàm ẩn danh (anonymous function) trong Python, được sử dụng để tạo ra các hàm nhỏ gọn mà không cần phải đặt tên. Cú pháp của hàm lambda như sau: 
...
`List comprehension` là một cách ngắn gọn để tạo ra một danh sách mới bằng cách áp dụng một biểu thức cho mỗi phần tử trong một iterable (như list, tuple, v.v.) và có thể kèm theo điều kiện lọc. Cú pháp của `list comprehension` như sau:

```bash
ketqua = [expression for item in iterable if condition]
```
...
Lớp (class) trong Python là một khuân mẫu, được sử dụng khi bạn muốn tổ chức, quản lý và tái sử dụng logic xoay quanh một đối tượng thay vì viết các hàm rời rạc. Mỗi đối tượng (object) được tạo từ class sẽ bao gồm thuộc tính (attributes) để lưu trữ dữ liệu và phương thức (methods) để thực hiện các thao tác liên quan đến dữ liệu đó.

### 5.3.1. Tạo lớp với phương thức và thuộc tính cơ bản

Trong lập trình hướng đối tượng (OOP), một class thường bao gồm hai thành phần chính là thuộc tính (attribute) và phương thức (method).

Phương thức là các hàm được định nghĩa trong class, dùng để mô tả hành vi hoặc chức năng của đối tượng. Trong khi đó, thuộc tính là các biến được khai báo bên trong class, dùng để lưu trữ dữ liệu hoặc trạng thái của đối tượng.
...
## 5.3.2. Tạo lớp với phương thức nâng cao và tính kế thừa
